...
try:
    ...
    torch.ops.load_library(str(distribution("dvattn").locate_file('')) + "/dvattn/dvatt.abi3.so")
except OSError:
    warnings.warn("Could not load dvatt.abi3.so file. Defaulting to slower eager realization.")


def dvattn_fused(queries: torch.Tensor, locations: torch.Tensor, keys: list[torch.Tensor], values: list[torch.Tensor],
                  scale: float, fragment_lengths, cosines: torch.Tensor, sines: torch.Tensor, *, rotated_queries, out):
    """Custom rope+attention kernel"""
    torch.ops.libdvatt.dvattn_fused(out, rotated_queries, scale, locations, queries.contiguous(), fragment_lengths, keys, values, cosines, sines)
    return out


def dvattn_sdpa(queries: torch.Tensor, locations: torch.Tensor, keys: list[torch.Tensor], values: list[torch.Tensor],
    ...
    torch.ops.libdvatt.dvattn_sdpa(out, scale, locations, queries.contiguous(), fragment_lengths, keys, values)
    return out


def dvattn_sdpa_pt(queries: torch.Tensor, locations: torch.Tensor, keys: list[torch.Tensor], values: list[torch.Tensor],
                    scale: float, return_intermediate_results: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Pure pytorch implementation. Useful for testing and debugging.
    """
    qk = []
    vals = []
...
def dvattn_rope(queries: torch.Tensor, cosines: torch.Tensor, sines: torch.Tensor, out=None):
    if out is None:
    ...
    torch.ops.libdvatt.dvattn_rope(out, queries, cosines, sines)
    return out
...
class AttentionModule(nn.Module):
    ...
    def forward(
            ...
            past_key_value: Optional[DVAttnCache] = None,
        ...
        if self.config._attn_implementation == "eager":
            ...
            attn_output = dvattn_sdpa_pt(queries, cache.loc, cache.keys, cache.values, self.scaling, False)
        else:
            rq = past_key_value.get_queries_buffer(query_states, layer_idx=self.layer_idx)
            attn_output = dvattn_fused(
                query_states,
                cache.loc,
                cache.keys,
                cache.values,
