...
__global__ __launch_bounds__(256) void dvattn_varlen_paged_attention_gpu_kernel21(
        ...
        const int* locations,         // [F, total_q_tokens]
        const scalar_t* queries,      // [F, total_q_tokens, Hq, E] (rotated)
        const int* fragment_lengths,  // [F]
        const scalar_t* k_cache,      // [num_blocks, Hkv, block_size, E]
        const scalar_t* v_cache,      // [num_blocks, Hkv, block_size, Ev]
        const int* block_tables,      // [W, max_blocks_per_seq]
        Shape shape) {
    ...
    int w = 0;
    // This loop is okay for moderate batch sizes. For very large batches, a binary search would be faster.
    for (int seq_idx = 0; seq_idx < shape.W; ++seq_idx) {
        if (token_idx < shape.cu_seqlens_q[seq_idx + 1]) {
            w = seq_idx;
            break;
        }
    }
    ...
    for (int f = 0; f < shape.F; ++f) {
        int q_loc = locations[f * shape.total_q_tokens + token_idx];
        if (q_loc < 0) {
            turn_token_offset += fragment_lengths[f];
            continue;
        }
        ...
        for (int gqa = 0; gqa < GQA; ++gqa) {
            for (int ee = 0; ee < VPH_k; ++ee) {
                ...
                ptrdiff_t q_offset = (((f * shape.total_q_tokens + token_idx) * Hq) + (hq + gqa)) * E;
                full_vec_t qv = full_vec_t::load(queries + q_offset + e);
    ...
    for (int gqa_offset = 0; gqa_offset < GQA; gqa_offset += (blockDim.x / WarpSize)) {
        ...
        int h = hkv * GQA + gqa;
        int res_base = h * shape.total_q_tokens + token_idx;
        int res_inc = shape.Hq * shape.total_q_tokens;
        int res_idx = res_base + split * res_inc;
        float* global_accumulator = reinterpret_cast<float*>(workspace);
        float* lse_target = global_accumulator + (size_t)shape.total_q_tokens * shape.Hq * Ev * splits;
...
__global__ __launch_bounds__(32) void dvattn_varlen_paged_attention_reduce_kernel(
    ...
    for (int split = 0; split < splits; ++split) {
        int res_idx = (h * shape.total_q_tokens + token_idx) + split * (shape.Hq * shape.total_q_tokens);
    ...
    for (int ee = 0; ee < Ev / warp.size(); ee += fvec_t::size) {
        ...
        st.store(out + ((size_t)token_idx * shape.Hq + h) * Ev + e);
    }
}

template<class scalar_t>
cudaError_t dvattn_varlen_paged_attention_gpu(
                           ...
                           Shape shape) {
    int problem_size = shape.Hkv * shape.total_q_tokens;
    ...
    size_t required_workspace_elems = (size_t)shape.Hq * shape.total_q_tokens * splits;
    size_t required_bytes = required_workspace_elems * ((size_t)shape.Ev + 1) * sizeof(float);
    ...
    if (shape.E == 128 && shape.Ev == 128 && (shape.Hq / shape.Hkv) == 16) {
        CUDA_RETURN_ON_ERROR(cudaFuncSetAttribute(dvattn_varlen_paged_attention_gpu_kernel21<128, 128, 16, scalar_t>,
        ...
        dim3 r_block_dim{32};
        dvattn_varlen_paged_attention_reduce_kernel<128><<<r_grid_dim, r_block_dim>>>(
                out, v_buffer, lse_buffer, splits, shape);
        CUDA_CHECK_THROW(cudaGetLastError());
    } else {
        printf("Unsupported head dimension for varlen paged attention kernel");
