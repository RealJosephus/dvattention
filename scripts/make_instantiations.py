...
#ifdef DVATTN_ENABLE_FLOAT
template cudaError_t {VERSION}::dvattn_attention_gpu<float>(
...
#ifdef DVATTN_ENABLE_HALF
template cudaError_t {VERSION}::dvattn_attention_gpu<half>(
...
#ifdef DVATTN_ENABLE_BFLOAT
template cudaError_t {VERSION}::dvattn_attention_gpu<nv_bfloat16>(
...
CMAKE_FILE_TEMPLATE = "SET(DVATTN_KERNEL_INSTANTIATIONS {SOURCES})"
...
with open("tpl/CMakeLists.txt", "w") as cml:
    cml.write("SET(DVATTN_KERNEL_INSTANTIATIONS\n")
...
cudaError_t dvattn_attention_gpu_dispatch(
    ...
    for version in versions:
        ...
        switch += f'return {version}::dvattn_attention_gpu(out, scale, locations, queries, fragment_lengths, key_fragments, value_fragments, shape);\n}}\n'
